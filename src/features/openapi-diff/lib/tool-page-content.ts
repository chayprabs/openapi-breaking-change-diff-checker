...
export const OPENAPI_DIFF_PAGE_PATH = "/" as const;
export const OPENAPI_DIFF_PAGE_TITLE = "OpenAPI Diff Online - Detect Breaking API Changes";
...
export const openApiDiffHowItWorks = [
  {
    ...
    body: "Paste, upload, or import two YAML or JSON API descriptions. The tool parses OpenAPI and Swagger documents, detects the spec version, and normalizes the contract before comparison starts.",
  },
  {
    title: "Compare API behavior semantically",
    body: "The diff engine looks at paths, operations, parameters, request bodies, responses, schemas, enums, and security requirements. Formatting churn and key order matter far less than actual contract behavior.",
  },
  {
    title: "Explain client impact in plain language",
    body: "Every finding is labeled as breaking, dangerous, safe, or docs-only and includes why it matters, affected paths, rule IDs, and severity summaries so reviewers can act on the result quickly.",
  },
  {
    title: "Turn review into repeatable workflow",
    body: "Once the browser report looks right, you can export PR-ready Markdown, standalone HTML, JSON, share a redacted report, or generate CI snippets for pull-request checks.",
  },
] as const;
...
export const openApiDiffBreakingSignals = [
  {
    ...
    body: "Adding a required query, path, header, or request-body field can make existing requests invalid even when the endpoint path stays the same.",
  },
  {
    title: "Removed output shape",
    body: "Removing a response field, media type, status code, or schema branch can break clients, SDKs, data pipelines, and UI code that still expect the old shape.",
  },
  {
    title: "Narrowed allowed values",
    body: "Removing enum values, tightening validation rules, or changing a schema type can invalidate real payloads that worked before the release.",
  },
  {
    title: "Security tightened in place",
    body: "Adding required auth scopes or changing security requirements can break calls at runtime even if the endpoint still exists and the shape looks similar.",
  },
  {
    title: "Endpoint or method removed",
    body: "Deleting a route, removing an operation, or dropping a previously supported success response is a classic backward-compatibility break.",
  },
  {
    title: "Version-specific edge cases",
    body: "OpenAPI 3.0.x and 3.1.x are supported, but the tool still warns when a schema uses unsupported or ambiguous keywords so you do not mistake uncertainty for safety.",
  },
] as const;
...
export const openApiDiffCommonUseCases = [
  {
    ...
    body: "Check whether a proposed API change should block the merge, trigger rollout coordination, or simply ship with a docs note.",
  },
  {
    title: "SDK and mobile release safety",
    body: "Catch changes that generated clients, slower mobile release cycles, or strict deserializers are likely to feel first.",
  },
  {
    title: "Partner and public API governance",
    body: "Review compatibility risk before changing externally consumed APIs where even small contract shifts can become support incidents.",
  },
  {
    title: "Upstream API monitoring",
    body: "Compare vendor or dependency specs across versions and export a report before you update integrations or regenerate models.",
  },
  {
    title: "Release notes and migration planning",
    body: "Use the risk summary, top breaking changes, and rule-level explanations to produce cleaner migration notes for downstream teams.",
  },
  {
    title: "Compliance and audit trails",
    body: "Keep machine-readable JSON, standalone HTML, and Markdown snapshots of what changed and why it was classified as risky or safe.",
  },
] as const;
...
export const openApiDiffBreakingVsSafeExamples = [
  {
    ...
    safeChange:
      "Adding `?region=` as an optional query parameter with a backward-compatible default.",
    why: "Existing callers already know how to call the operation. Requiring a new input breaks them; offering an optional input usually does not.",
  },
  {
    title: "Response fields",
    breakingChange:
      "Removing `customer.email` from the response payload clients already deserialize.",
    safeChange:
      "Adding a new optional response field such as `customer.nickname` while keeping existing fields unchanged.",
    why: "Consumers often read or persist old response fields. Returning more data is usually safer than returning less, though strict clients can still need review.",
  },
  {
    title: "Enums",
    breakingChange:
      "Removing the enum value `suspended` from `accountStatus` when existing records may still use it.",
    safeChange:
      "Adding a new enum value while coordinating with strict consumers that need an explicit allow-list update.",
    why: "Removing accepted values invalidates old data immediately. Adding values is often safer, but some clients still need rollout planning, so the tool may classify it as dangerous instead of fully safe.",
  },
  {
    title: "Schema evolution",
    breakingChange: "Changing `creditLimit` from `integer` to `string` in place.",
    safeChange:
      "Introducing `creditLimitText` or a versioned schema while leaving the original field untouched.",
    why: "In-place type changes break validation, parsing, SDK generation, and stored assumptions. Additive versioning preserves compatibility.",
  },
] as const;
...
export const openApiDiffSemanticDiffBenefits = [
  {
    ...
    body: "A plain git diff cannot tell the difference between harmless reformatting and a real contract break. Semantic diffing compares meaning, not line churn.",
  },
  {
    title: "Client-impact explanations",
    body: 'The tool does not stop at "changed." It explains who is likely to break, why the change matters, and which rule classified it.',
  },
  {
    title: "Profile-aware review",
    body: "Public APIs, internal clients, mobile apps, and strict SDK consumers do not all feel risk the same way. The selected profile changes how additive findings are framed.",
  },
  {
    title: "Actionable exports",
    body: "You can move from browser exploration to GitHub PR comments, HTML handoff reports, JSON automation, share links, and CI snippets without rebuilding the analysis by hand.",
  },
] as const;
...
export const openApiDiffPrivacyNotes = [
  {
    ...
    body: "Core parsing, normalization, and semantic diffing run in a Web Worker in your browser so the main analysis flow does not depend on storing pasted or uploaded specs on the server.",
  },
  {
    title: "Safe proxy only for public URL fetches",
    body: "When you import a public URL or allow public remote refs, the app tries a browser fetch first. If the browser cannot fetch the document directly, it falls back to a restricted no-store proxy that blocks localhost, private IP ranges, metadata endpoints, authenticated URLs, and dangerous redirects.",
  },
  {
    title: "No raw spec storage by default",
    body: "Settings can persist in localStorage, but raw editor contents are remembered only when you explicitly opt in. Share links omit raw specs, and report links require redaction before they can be generated.",
  },
] as const;
...
export const openApiDiffRuleExplainers = [
  {
    ...
    body: "This rule fires when an operation introduces a new required parameter that callers did not have to send before. Even if the endpoint path is unchanged, existing requests can start failing validation.",
    exampleAfter: "parameters:\n  - in: query\n    name: region\n    required: true",
    exampleBefore: "parameters:\n  - in: query\n    name: includeOrders\n    required: false",
  ...
  },
  {
    ...
    body: 'When a response schema drops a field that consumers already read, the tool typically reports `schema.property.removed`. The concept is "response property removed," even though the rule is attached to the affected schema node.',
  ...
  },
  {
    ...
    body: "Removing an enum value is a backward-incompatible change whenever existing payloads, database rows, or client logic still rely on that value being accepted.",
    exampleAfter: "enum:\n  - active\n  - closed",
    exampleBefore: "enum:\n  - active\n  - suspended\n  - closed",
  ...
  },
  {
    ...
    body: "Changing a schema type in place, such as `integer` to `string`, often breaks validation, serialization, generated clients, and code paths that assume the old runtime type.",
  ...
  },
  {
    ...
    body: "Adding a required OAuth scope can break clients at authorization time even if request and response shapes still look compatible. That is why the tool flags it separately from pure schema changes.",
    exampleAfter: "security:\n  - oauth2:\n      - accounts:read\n      - accounts:export",
...
export function buildOpenApiDiffStructuredData(path: string = OPENAPI_DIFF_PAGE_PATH) {
